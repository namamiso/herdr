...
mod tests {
    ...
    #[test]
    fn save_agent_panel_scope_persists_and_keeps_sort_without_duplicate_ui_section() {
        let _guard = config_env_lock().lock().unwrap();
        let path = temp_config_path("save-agent-panel-scope");
        std::fs::create_dir_all(path.parent().unwrap()).unwrap();
        // Pre-existing [ui] section already carries a sort setting.
        std::fs::write(
            &path,
            "onboarding = false\n[ui]\nagent_panel_sort = \"priority\"\n",
        )
        ...
        // The sibling sort setting must survive the scope write.
        assert!(content.contains("agent_panel_sort = \"priority\""));
        // The [ui] header must not be duplicated.
        assert_eq!(content.matches("[ui]").count(), 1);
        assert!(app.state.config_diagnostic.is_none());
