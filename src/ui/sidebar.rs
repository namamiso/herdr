...
fn agent_panel_entries_with_runtimes(
    ...
    apply_agent_panel_scope(app, &mut entries);
    crate::app::agent_view::apply_agent_view(app, &mut entries);
    entries
}

/// Scope decides the listed set BEFORE sorting, and only for the panel's
/// default view: an active agent view override defines the canonical entry
/// set itself, so scope is skipped while one is set. In `current` scope the
...
fn apply_agent_panel_scope(app: &AppState, entries: &mut Vec<AgentPanelEntry>) {
    ...
    if matches!(app.agent_panel_scope, AgentPanelScope::CurrentWorkspace) {
        ...
        entries.retain(|entry| Some(entry.ws_idx) == current_idx);
        for entry in entries.iter_mut() {
            entry.primary_label = String::new();
        }
    }
}
...
fn agent_panel_rows_with_runtimes(
    ...
    if app.agent_view_override.is_none() {
        let mut plain_tabs = collect_plain_tab_entries_with_runtimes(app, terminal_runtimes);
        apply_agent_panel_scope(app, &mut plain_tabs);
...
fn render_agent_detail(
    ...
    if let Some(view_label) = active_agent_view_label(app) {
        // An active agent view override replaces the directory/scope/sort
        // controls with its own label; the toggles are not clickable then.
        let toggle_rect = agent_panel_header_label_rect(area, view_label);
