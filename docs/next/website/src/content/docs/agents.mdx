...
In the app you can also name the focused agent through the pane rename action (or the pane context menu), which shows **name agent** for agent panes. The name leads the Agent panel entry, with the detected agent type kept beside it (see the [`agent` and `agent_type` tokens](/docs/configuration/#sidebar-row-layouts)).
