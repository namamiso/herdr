...
row_gap = 0
rows = [
  ["state_icon", "agent", { token = "agent_type", dim = true }],
  ["workspace", "tab"],
]
...
- `agent` — detected or reported agent display name, or the custom name you give it with **name agent** (see below).
- `agent_type` — the detected agent type such as `claude` or `codex`, shown only when the agent has a custom name so its type stays visible beside the name. Without a custom name it disappears, because `agent` already shows the type.
