...
- `tab` — 利用可能な場合はタブ名。
- `pane` — 利用可能な場合はペイン名。
- `agent` — 検出または報告されたエージェントの表示名。**name agent** で付けたカスタム名がある場合はそれ（下記参照）。
- `agent_type` — `claude` や `codex` などの検出されたエージェント型。カスタム名が付いているときだけ、名前の横に型を残すために表示されます。カスタム名がないときは `agent` が型を示すため非表示になります。
