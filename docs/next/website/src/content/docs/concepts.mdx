...
The sidebar agent panel lists agents first, then a `tabs` group with tabs that have no agent panes, so plain shell tabs stay one click away. Clicking a row activates its workspace and tab.
