...
The sidebar agent panel lists agents first, then a `tabs` group with tabs that have no agent panes, so plain shell tabs stay one click away. Clicking a row activates that tab. An active agent view filters the agent rows only; the `tabs` group keeps listing every agent-less tab.
